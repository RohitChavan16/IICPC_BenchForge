...
import { mockBenchmarkSessions } from '@/data/mockDashboardData';
...
export function BenchmarkSessionsTimeline() {
  const data = mockBenchmarkSessions;
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-muted-foreground text-xs font-mono px-2 py-0.5 bg-background border border-border rounded-full shadow-sm">{session.submission}</span>
                  </div>
                  <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                     ...
                     {session.replay === 'Available' && <button className="px-2 py-1 text-xs rounded bg-primary text-primary-foreground hover:bg-primary/90 flex items-center gap-1 font-medium"><Play size={12}/> Replay</button>}
                  </div>
                </div>
                
                <div className="flex flex-wrap items-center gap-4 text-xs text-muted-foreground">
                  <span className="flex items-center gap-1"><Clock size={14} /> {session.duration}</span>
                  <span className="flex items-center gap-1"><Server size={14} /> {session.workers} Workers</span>
                  <span className="flex items-center gap-1"><Users size={14} /> {session.personaMix} Mix</span>
                  {session.replay === 'Available' && <span className="flex items-center gap-1 text-purple-500"><PlayCircle size={14} /> Replay Ready</span>}
                </div>
              </div>
            </div>
          );
