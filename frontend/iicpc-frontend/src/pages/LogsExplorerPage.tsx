  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <p className="text-sm uppercase tracking-[0.3em] text-primary/80">Log intelligence</p>
          <h1 className="mt-2 text-3xl font-semibold text-foreground">Logs explorer</h1>
        </div>
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
          ...
          <div className="flex items-center gap-2 rounded-3xl border border-border bg-background px-4 py-3 text-sm text-muted-foreground">
            <Filter size={16} />
            <select
              value={level}
      ...
      <Card title="Logs" description="Search and filter platform logs in real time.">
        <div className="rounded-[32px] border border-border bg-background p-4">
          <div className="grid gap-4">
            {(logs ?? []).map((log) => (
              <div key={log.id} className="rounded-3xl border border-border bg-card p-4">
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <div className="space-y-1">
                    <p className="text-sm font-semibold text-foreground">{log.source}</p>
                    <p className="text-xs text-foreground0">{new Date(log.timestamp).toLocaleString()}</p>
                  </div>
                  <Badge variant={log.level === 'error' ? 'danger' : log.level === 'warn' ? 'warning' : 'info'}>{log.level}</Badge>
                </div>
                <p className="mt-3 text-sm leading-6 text-muted-foreground">{log.message}</p>
                <p className="mt-3 text-xs uppercase tracking-[0.24em] text-foreground0">{log.service}</p>
              </div>
            ))}
          </div>
        </div>
